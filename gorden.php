	...
	if(isset($_GET['action']))
	{
		if($_GET['action']=='nuevo')
		{
			...
			if(mysql_num_rows($result2)>0)
			{
				...
				$wsql="INSERT INTO ordenservicio (numero_orden,hora_estimada,estatus,cedula_paciente,idempresa,idsucursal,idperfil) VALUES 					('$numerorden','$hora_estimada','Pendiente','$cedulag','$idemp','$laboratorio','$perfil')";
				$result = mysql_query($wsql);
				//header("location:contacto.php?pag=vo&&acc=pac&&tipo=svo");
		...
		if($_GET['action']=='anular')
		{
			...
			$wsql="SELECT * FROM ordenservicio WHERE idordenservicio='$id_orden'";
			...
			if($hora_actual>$row['hora_estimada'])
			{
				...
				$wsql="UPDATE ordenservicio SET estatus='Anulada' WHERE idordenservicio='$id_orden'";
				mysql_query($wsql);
		...
		if($_GET['action']=='modificar')
		{
			...
			$laboratorio = $_POST["sucursal"];
			$hora = time();
			$wsql="UPDATE ordenservicio SET idperfil='$perfil', idsucursal='$laboratorio' WHERE idordenservicio='$id_orden'";
			$result = mysql_query($wsql);
